...
🎥 **[Watch the 90-second demo](PASTE_LOOM_LINK_HERE)**
...
The agent was tested against 10 analytical questions of increasing difficulty (aggregates → multi-table joins → date math → utilization metrics). Full log in [`evals/questions.md`](evals/questions.md).

| Version | Score | Notes |
|---|---|---|
| v1 — plain schema prompt | X/10 | FILL: e.g. failed on month grouping and date ranges |
| v2 — after prompt iteration | Y/10 | FILL: what you changed, e.g. added example rows + one worked example |

**Most interesting failure:** FILL — e.g. "monthly rental counts" grouped by day instead of month; fixed by adding an explicit strftime example to the prompt.
